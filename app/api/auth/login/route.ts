...
export async function POST(request: Request) {
  try {
    ...
    await db.activity.create({
      data: { userId: user.id, type: 'auth', title: 'Đăng nhập', detail: 'Đăng nhập vào CareerTwin' },
    });
    return Response.json({
      ok: true,
      user: { id: user.id, name: user.name, email: user.email, onboardingCompleted: Boolean(user.onboardingCompletedAt) },
    });
  } catch (error) {
